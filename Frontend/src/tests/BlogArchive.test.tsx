...
describe('BlogArchive Component', () => {
  ...
  describe('Utility Functions', () => {
    ...
    it('uses Admin as default author when author is missing', async () => {
      const noAuthorBlog = {
        ...
        // Note: No author property at all
      }
      ...
      await waitFor(() => {
        expect(screen.getByText('No Author Blog')).toBeInTheDocument()
      })

      // Check if Admin appears in the author section
      await waitFor(() => {
    ...
    describe('Image Handling - Comprehensive Coverage', () => {
      it('handles blog with null image', async () => {
        const nullImageBlog = {
          _id: '1',
          title: 'Null Image Blog',
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [nullImageBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          expect(screen.getByText('Null Image Blog')).toBeInTheDocument()
        })

        // Check that some image is rendered (either fallback or placeholder)
        await waitFor(() => {
          const images = screen.getAllByRole('img')
          const relevantImages = images.filter(img =>
            img.getAttribute('alt') === 'Null Image Blog' ||
            img.closest('.blog-img') ||
            img.closest('.content-img')
          )
          expect(relevantImages.length).toBeGreaterThan(0)
        })
      })

      it('handles blog with undefined image', async () => {
        const undefinedImageBlog = {
          _id: '1',
          title: 'Undefined Image Blog',
          content: 'Content here',
          author: 'Test Author',
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [undefinedImageBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          expect(screen.getByText('Undefined Image Blog')).toBeInTheDocument()
        })
      })

      it('handles blog with empty string image', async () => {
        const emptyImageBlog = {
          _id: '1',
          title: 'Empty Image Blog',
          content: 'Content here',
          author: 'Test Author',
          image: '', // Empty string
          createdAt: '2024-01-01T10:00:00.000Z'
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [emptyImageBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          expect(screen.getByText('Empty Image Blog')).toBeInTheDocument()
        })
      })
    })
  ...
  describe('Edge Cases', () => {
    ...
    describe('Pagination Edge Cases', () => {
      it('handles previous button click on first page (should not change page)', async () => {
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: mockBlogs, totalPages: 3 }
        })
        ...
        // Try to click previous button on first page
      ...
      it('handles next button click on last page (should not change page)', async () => {
        vi.mocked(api.get)
          .mockResolvedValueOnce({ data: { blogs: mockBlogs, totalPages: 2 } })
          .mockResolvedValueOnce({ data: { blogs: [mockBlogs[0]], totalPages: 2 } })
        ...
        await waitFor(() => {
          // Now on page 2, try to click next
      ...
      it('handles click on current active page', async () => {
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: mockBlogs, totalPages: 3 }
        })
        ...
        await waitFor(() => {
          ...
          if (activePage) {
            userEvent.click(activePage)
            // Should stay on same page but still make API call
            expect(api.get).toHaveBeenCalledWith('/blogs?page=1')
          }
        })
      })
    ...
    describe('Content Rendering Edge Cases', () => {
      it('handles blog with empty string content', async () => {
        const emptyContentBlog = {
          _id: '1',
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [emptyContentBlog], totalPages: 1 }
        })
      ...
      it('handles blog with undefined createdAt date', async () => {
        const noDateBlog = {
          _id: '1',
          title: 'No Date Blog',
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [noDateBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          expect(screen.getByText('No Date Blog')).toBeInTheDocument()
          // Should handle undefined date gracefully
        })
      })

      it('handles blog with invalid date string', async () => {
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [invalidDateBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          expect(screen.getByText('Invalid Date Blog')).toBeInTheDocument()
          // Should handle invalid date gracefully
        })
      })

      it('handles blog with very short content (less than 150 characters)', async () => {
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [shortContentBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          const excerptText = screen.getByText('Short content.')
          // Should not have ... appended since it's short
          expect(excerptText.textContent).not.toMatch(/\.\.\.$/)
        })
      })

      it('handles blog with exactly 150 character content', async () => {
        const exactLengthContent = 'A'.repeat(150)
        const exactLengthBlog = {
          _id: '1',
          title: 'Exact Length Blog',
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [exactLengthBlog], totalPages: 1 }
        })

        render(<BlogArchive />)

        await waitFor(() => {
          const excerptText = screen.getByText(exactLengthContent)
          // Should not have ... appended since it's exactly 150
          expect(excerptText.textContent).toBe(exactLengthContent)
        })
      })
    })

    describe('Image Handling Edge Cases', () => {
      it('handles blog with empty string image path', async () => {
        const emptyImageBlog = {
          _id: '1',
          title: 'Empty Image Blog',
          content: 'Content',
      ...
      it('handles blog with whitespace-only image path', async () => {
        const whitespaceImageBlog = {
          _id: '1',
          title: 'Whitespace Image Blog',
          content: 'Content',
          author: 'Test Author',
          image: '   ',
          createdAt: '2024-01-01T10:00:00.000Z'
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [whitespaceImageBlog], totalPages: 1 }
        })
        ...
        await waitFor(() => {
          ...
          const images = screen.getAllByRole('img')
          const blogImages = images.filter(img =>
            img.closest('.blog-content') || // Look within blog content area
            img.getAttribute('alt')?.includes('Blog') ||
            img.getAttribute('src')?.includes('img17.jpg')
          )
          expect(blogImages.length).toBeGreaterThan(0)
    ...
    describe('Author Handling Edge Cases', () => {
      it('handles blog with empty string author', async () => {
        const emptyAuthorBlog = {
          _id: '1',
          title: 'Empty Author Blog',
          content: 'Content',
          author: '',
          createdAt: '2024-01-01T10:00:00.000Z'
        }
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [emptyAuthorBlog], totalPages: 1 }
        })
      ...
      it('handles blog with whitespace-only author', async () => {
        const whitespaceAuthorBlog = {
          _id: '1',
          title: 'Whitespace Author Blog',
          content: 'Content',
          author: '   ',
          createdAt: '2024-01-01T10:00:00.000Z'
        }
        ...
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [whitespaceAuthorBlog], totalPages: 1 }
        })
    ...
    describe('Pagination Boundary Tests', () => {
      it('handles totalPages equal to 0', async () => {
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [], totalPages: 0 }
        })
      ...
      it('handles negative totalPages', async () => {
        vi.mocked(api.get).mockResolvedValue({
          data: { blogs: [], totalPages: -1 }
        })
